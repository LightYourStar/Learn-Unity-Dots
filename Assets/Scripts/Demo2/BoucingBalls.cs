...
{
    [Header("数量与显示")]
    [Range(1000, 100000)] public int ballCount = 50000;
    ...
    [Header("性能切换")]
    public bool useParallel = true;        // 对比 Schedule 与 ScheduleParallel
    ...
    private struct MoveAndBounceJob : IJobParallelFor
    {
        ...
        public void Execute(int i)
        {
            ...
            if (p.x < xmin) { p.x = xmin; v.x = -v.x; }
            else if (p.x > xmax) { p.x = xmax; v.x = -v.x; }

            // 上下
            float ymin = boundsXYXY.y + radius;
            float ymax = boundsXYXY.w - radius;
            if (p.y < ymin) { p.y = ymin; v.y = -v.y; }
            else if (p.y > ymax) { p.y = ymax; v.y = -v.y; }
